---
title: "England R and growth rate estimates"
date: today
...
R and growth rate estimates were published in real time from May 2020. A final dataset was published 7 December 2022^[https://www.gov.uk/government/publications/reproduction-number-r-and-growth-rate-methodology]. These estimates are:
...
Published data tables include lower and upper bound estimates for R and the growth rate per day (90% Confidence Interval), for England and the seven NHS England regions. Data were downloaded and lightly pre-processed.
...
```{r}
plot_estimates <- function(region, data) {
    data |>
filter(region == {{region}}) |>
mutate(reference = ifelse(value_type == "R", 1, 0)) |>
ggplot(aes(x = date, col = model, fill = model)) +
geom_line(aes(y = value_lower), alpha = 0.4) +
geom_line(aes(y = value_upper), alpha = 0.4) +
geom_ribbon(aes(ymin = value_lower, ymax = value_upper), alpha = 0.3) +
...
Using Rt and growth rate from inc2prev.

```{r}
i2p_nation <- read_csv("https://github.com/epiforecasts/inc2prev/raw/refs/heads/main/outputs/estimates_national.csv") |>
filter(variable == "England")
i2p_region <- read_csv("https://raw.githubusercontent.com/epiforecasts/inc2prev/refs/heads/main/outputs/estimates_regional.csv")

i2p <- bind_rows(i2p_nation, i2p_region) |>
select(date, region = variable, value_type = name, value_lower = q10, value_upper = q90) |>
filter(value_type %in% c("R", "r")) |>
mutate(value_type = ifelse(value_type == "r", "growth rate", "R"))
knitr::kable(head(i2p))
```

Plotting directly against consensus estimates.

```{r}
i2p <- mutate(i2p, model = "i2p")
comp <- bind_rows(i2p, r_gr)
plot_estimates("England", comp)
```

But i2p is calculated daily. 
We should keep only the days that overlap with the consensus, or take an average across the week covered by the consensus estimate.
...
```{r output="asis"}
regions <- unique(r_gr$region)
for (i in regions) {
    cat("\n::: {.callout-note collapse=\"true\"}")
    cat("\n####", i, "\n")
    print(plot_estimates(i, r_gr))
    cat("\n:::\n")
}
```
