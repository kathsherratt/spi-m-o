...
```{r}
plot_estimates <- function(region) {
    r_gr |>
...
geom_line(aes(y = reference), lty=2) +
facet_wrap(~value_type, ncol=1, scales = "free_y") +
labs(x = NULL, y = "Weekly estimate (90% CI)",
title = paste({{region}}, "COVID-19 epidemic indicators"), subtitle = "UK government public estimates of R and growth rate")
}
plot_estimates("England")
```
...
- Comparison with retrospectively calculated R and GR using latest available methodology and data
    - Calcuation of R from epinowcast
    - Data on infections from ONS incidence survey; or latest UKHSA estimates of deaths within 28 days and latest best estimates of delay from infection through death (if time-varying)
